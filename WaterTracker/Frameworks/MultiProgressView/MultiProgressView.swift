...
open class MultiProgressView: UIView {
    ...
    public func setProgress(section: Int, progress: Float, isReloadColor: String? = nil) {
        currentProgress[section] = max(0, min(progress, 1 - totalProgress + currentProgress[section]))
        
        progressViewSections[section].setColor(isReloadColor)
        
        setNeedsLayout()
        layoutIfNeeded()
    }

    public func getCountProgress(_ index: Int = 0) -> Float {
        var countProgress: Float = 0
        
        for section in index..<progressViewSections.count {
            countProgress += currentProgress[section]
        }
        
        return countProgress
    }
    
    public func resetProgress(_ index: Int = 0) {
        for section in index..<progressViewSections.count {
            setProgress(section: section, progress: 0)
        }
    }
}
